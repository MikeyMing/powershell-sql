...
function BackupAndRestore {
    <#
    .SYNOPSIS
        Copies/clones a SQL Server database via backup/restore.

    .DESCRIPTION
        Performs a SQL Server BACKUP of a source database and RESTOREs it to a target instance as a
        new database (default) or over an existing database (when -CreateDatabase $false).

        Supports filesystem/UNC paths via -BackupPath and Azure Blob Storage via
        -AzureStorageBackupLocation (container URL with optional SAS token).

        This command has many parameters for common DBA workflows. For safe iteration, start with
        -PreflightOnly or -DryRun to validate inputs before making changes.

    .NOTES
        Common scenarios (see examples below):
        - Clone to a new database (safe default)
        - Overwrite an existing database (destructive)
        - Azure Blob Storage backup/restore
        - Enable SQL-backed logging (-EnableDbLogging)
        - PreflightOnly / DryRun for validation

    .PARAMETER SourceInstance
        Source SQL Server instance name (e.g. 'SERVER\INSTANCE').
    ...
    .PARAMETER BackupPath
        Filesystem or UNC directory used to write/read backups.
        For multi-server scenarios, prefer a UNC path (network share) that the SQL Server service account(s) can access.
        Mutually exclusive with -AzureStorageBackupLocation.

    .PARAMETER AzureStorageBackupLocation
        Azure Blob container URL (optionally with SAS token) used as the backup/restore location.
        Mutually exclusive with -BackupPath.
    ...
    .PARAMETER TakeTargetOffline
        When $true, attempts to take the target database offline before restore.

    .PARAMETER EnableDbLogging
        Enables SQL-backed logging to dbo.Log in the configured DBA database.
        Configure via Set-DBALibraryConfig.

    .PARAMETER VerifyBackup
        Runs RESTORE VERIFYONLY against the produced backup before restoring.

    .PARAMETER PreflightOnly
        Validates parameters and environment checks, then exits before running backup/restore.
    ...
    .EXAMPLE
        BackupAndRestore -SourceInstance 'SERVER\INSTANCE' -SourceDatabase 'MyDb' \
          -TargetInstance 'SERVER\INSTANCE' -TargetDatabase 'MyDb_Copy' \
                    -BackupPath '\\fileserver\sqlbackups\staging' -BatchMode

        Clones a database to a new target database using a local/UNC backup path.

    .EXAMPLE
        BackupAndRestore -SourceInstance 'SERVER\INSTANCE' -SourceDatabase 'MyDb' \
          -TargetInstance 'SERVER\INSTANCE' -TargetDatabase 'MyDb' \
                    -BackupPath '\\fileserver\sqlbackups\staging' -CreateDatabase $false -BatchMode -TakeTargetOffline $true

        Overwrites an existing target database (destructive).

    .EXAMPLE
        $containerUrlWithSas = 'https://<account>.blob.core.windows.net/<container>?sv=...&sig=...'
        BackupAndRestore -SourceInstance 'SERVER\INSTANCE' -SourceDatabase 'MyDb' \
          -TargetInstance 'SERVER\INSTANCE' -TargetDatabase 'MyDb_Copy' \
          -AzureStorageBackupLocation $containerUrlWithSas -BatchMode

        Uses Azure Blob Storage as the backup/restore location.

        .EXAMPLE
                Set-DBALibraryConfig -DBAInstance 'SERVER\INSTANCE' -DBADatabase 'DBA'
                BackupAndRestore -SourceInstance 'SERVER\INSTANCE' -SourceDatabase 'MyDb' \
                    -TargetInstance 'SERVER\INSTANCE' -TargetDatabase 'MyDb_Copy' \
                    -BackupPath '\\fileserver\sqlbackups\staging' -EnableDbLogging -BatchMode

                Enables SQL-backed logging (writes to dbo.Log in the configured DBA database).

        .EXAMPLE
                BackupAndRestore -SourceInstance 'SERVER\INSTANCE' -SourceDatabase 'MyDb' \
                    -TargetInstance 'SERVER\INSTANCE' -TargetDatabase 'MyDb_Copy' \
                    -BackupPath '\\fileserver\sqlbackups\staging' -VerifyBackup -PreflightOnly -BatchMode

                Runs validations and verifies the backup, but skips the actual backup/restore execution.

        .EXAMPLE
                BackupAndRestore -SourceInstance 'SERVER\INSTANCE' -SourceDatabase 'MyDb' \
                    -TargetInstance 'SERVER\INSTANCE' -TargetDatabase 'MyDb_Copy' \
                    -BackupPath '\\fileserver\sqlbackups\staging' -DryRun -BatchMode

                Logs intended operations without performing destructive actions.
    #>
    [CmdletBinding()]
    param (
